    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (mState.LeftButton == ButtonState.Pressed && mReleased == true)
            {
                ...
                if (mouseTargetDist < 17 && timer > 0)
                {
                    score += 3;                    
                    targetPosition.X = rand.Next(0, (_graphics.PreferredBackBufferWidth - 90));
                    targetPosition.Y = rand.Next(0, (_graphics.PreferredBackBufferHeight - 90));
                } else if (mouseTargetDist >= 17 && mouseTargetDist < 30 && timer > 0)
                {
                    score += 2;
                    targetPosition.X = rand.Next(0, (_graphics.PreferredBackBufferWidth - 90));
                    targetPosition.Y = rand.Next(0, (_graphics.PreferredBackBufferHeight - 90));
                } else if (mouseTargetDist >= 30 && mouseTargetDist <= targetRadius && timer > 0)
                {
                    score++;
                    targetPosition.X = rand.Next(0, (_graphics.PreferredBackBufferWidth - 90));
                    targetPosition.Y = rand.Next(0, (_graphics.PreferredBackBufferHeight - 90));
